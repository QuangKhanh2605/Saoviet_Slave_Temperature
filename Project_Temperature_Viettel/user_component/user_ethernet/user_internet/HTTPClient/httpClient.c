	...
	switch(state)
	{
		...
		case SOCK_ESTABLISHED:
			if(getSn_IR(httpsock) & Sn_IR_CON)
			{
				...
				APP_LOG(TS_OFF, DBLEVEL_M, " > HTTP CLIENT: CONNECTED TO - %d.%d.%d.%d : %d\r\n",
		...
		case SOCK_CLOSED:
...
#ifdef _HTTPCLIENT_DEBUG_
			APP_LOG(TS_OFF, DBLEVEL_M, " > HTTP CLIENT: source_port = %d\r\n", source_port);
#endif
			...
			if (socket(httpsock, Sn_MR_TCP, source_port, Sn_MR_ND) == httpsock)
			{
				...
				{
#ifdef _HTTPCLIENT_DEBUG_
					APP_LOG(TS_OFF, DBLEVEL_M, " > HTTP CLIENT: SOCKOPEN\r\n");
#endif
...
#ifdef _HTTPCLIENT_DEBUG_
		APP_LOG(TS_OFF, DBLEVEL_M, " >> HTTP Request header - Method: %s, URI: %s, Content-Length: %d\r\n", req->method, req->uri, content_len);
		APP_LOG(TS_OFF, DBLEVEL_M, "%s", buf);
#endif
...
uint16_t httpc_send_body(uint8_t * buf, uint16_t len)
{
	...
	if(httpc_isConnected == HTTPC_TRUE)
	{
		...
		if(sentlen > 0)
		{
			APP_LOG(TS_OFF, DBLEVEL_M, " >> HTTP Request Body\r\n");
			for(i = 0; i < sentlen; i++) 
                APP_LOG(TS_OFF, DBLEVEL_M, "%c", buf[i]);
...
//#ifdef _HTTPCLIENT_DEBUG_
		APP_LOG(TS_OFF, DBLEVEL_M, " >> HTTP Request - Method: %s, URI: %s, Content-Length: %d\r\n", req->method, req->uri, content_len);
		for(i = 0; i < len; i++) 
            APP_LOG(TS_OFF, DBLEVEL_M, "%c", buf[i]);
...
uint8_t httpc_disconnect(void)
{
	...
	{
#ifdef _HTTPCLIENT_DEBUG_
		APP_LOG(TS_OFF, DBLEVEL_M, " > HTTP CLIENT: Try to disconnect\r\n");
#endif
		...
		if(ret == SOCK_OK)
		{
			ret = HTTPC_TRUE;
#ifdef _HTTPCLIENT_DEBUG_
			APP_LOG(TS_OFF, DBLEVEL_M, " > HTTP CLIENT: Disconnected\r\n");
#endif
		}
	}
